...
const App: React.FC = () => {
  ...
  // Bring in the store to power the CLI
  ...
  const handleCommandSubmit = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && searchQuery.startsWith('/')) {
      ...
      if (amount > 0) {
        addTransaction({
          amount,
          type: searchQuery.toLowerCase().includes('debit') ? 'debit' : 'credit',
          category: 'Operating',
          entity: 'CLI System',
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-[#050505] relative z-10">
          ...
          <header className="h-20 border-b border-white/5 flex items-center justify-between px-8 shrink-0 z-20 bg-[#0a0a0a]/80 backdrop-blur-xl relative">
            <div className={`flex items-center gap-3 bg-[#111113] border transition-all rounded-xl px-4 py-2 w-[450px] shadow-inner relative overflow-hidden ${searchFocused ? (isCommandMode ? 'border-indigo-500/50 ring-1 ring-indigo-500/20' : 'border-emerald-500/50 ring-1 ring-emerald-500/20') : 'border-white/5'}`}>
              ...
              <input 
                ...
                placeholder="Search ledgers, or type '/entry 5000' for CLI..." 
