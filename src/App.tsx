...
import { 
  Aperture, Inbox, LayoutDashboard, CheckSquare, 
  Users, Settings, Search, Command, Edit
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  return (
    <div className="flex h-screen w-screen bg-[#0E0E11] text-zinc-200 overflow-hidden font-sans antialiased selection:bg-indigo-500/30">
      ...
      {/* Left Sidebar */}
      <aside className="w-64 border-r border-zinc-800/60 bg-[#151517] flex flex-col flex-shrink-0 z-10 shadow-2xl">
        <div className="h-14 flex items-center px-4 border-b border-zinc-800/60 gap-3 hover:bg-zinc-800/30 cursor-pointer transition-colors">
          <div className="w-6 h-6 rounded-md bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-lg shadow-indigo-500/20 ring-1 ring-white/10">
        ...
        <div className="p-3">
          <button 
            ...
            className="w-full flex items-center justify-between px-3 py-2 bg-zinc-900 border border-zinc-800 rounded-lg hover:border-zinc-700 hover:bg-zinc-800/50 transition-all text-zinc-400 group shadow-sm"
          >
            <div className="flex items-center gap-2">
              <Search className="w-4 h-4" />
              <span className="text-sm font-medium group-hover:text-zinc-200 transition-colors">Search issues</span>
            </div>
        ...
        <nav className="flex-1 px-3 py-2 flex flex-col gap-0.5 overflow-y-auto no-scrollbar">
          <a href="#" className="flex items-center justify-between px-3 py-2 text-sm font-medium text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800/50 rounded-lg transition-colors group">
            <div className="flex items-center gap-3"><Edit className="w-4 h-4" /> New Issue</div>
            <span className="text-[10px] font-mono opacity-0 group-hover:opacity-100 transition-opacity">C</span>
          </a>
          
          <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest px-3 py-2 mt-4">Your Views</div>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#0E0E11] relative">
        <div className="absolute top-0 left-0 right-0 h-[250px] bg-gradient-to-b from-indigo-500/10 to-transparent pointer-events-none" />
        ...
        <header className="h-14 border-b border-zinc-800/60 flex items-center justify-between px-6 shrink-0 z-10">
          ...
          <button className="bg-white hover:bg-zinc-200 text-black px-4 py-1.5 rounded-md font-semibold text-xs transition-colors shadow-sm flex items-center gap-2">
            <Edit className="w-3 h-3" /> New Issue
          </button>
        </header>
